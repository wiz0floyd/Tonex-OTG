...
data class ConnectionTimeouts(
    ...
    companion object {
         ...
         * | [ConnectionTimeouts.transportWriteMillis] | 1000 | **Not a protocol delay at all.** [dev.tonexotg.protocol.TonexTransport.write]'s KDoc permits it to "suspend for as long as the underlying transport needs"; without a bound, one wedged write hangs `setParameter` forever. This bounds the seam, not the pedal. **Caveat, found during S11/PR #62 review:** `withTimeout(transportWriteMillis)` always *requests* cancellation of `write()` promptly, but a concrete implementation is not guaranteed to actually *return control* to the caller within this budget on the cancelled path — the Android `TonexUsbTransport` (S11), specifically, blocks the calling coroutine for up to ~3s past cancellation while it waits (deliberately, to close a different race — see its `write()` KDoc) for the underlying platform call to finish before letting the `CancellationException` propagate. This field still bounds how long `writeFramed` *waits before asking* to move on; it does not currently bound how long the calling thread can be occupied honoring that request against every transport. |
         *
