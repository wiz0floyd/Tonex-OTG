...
class SingleParameterPayloadCodecTest {
    ...
    // ---- decode: read-side byte layout, confirmed against real hardware (#106 / issue #104) --------

    @Test
    fun `decode reads kind index and value from the byte-exact hardware literal - issue 104 AC`() {
        // Literal frame from #106's hand-decoded hardware capture: B9 04 02 00 14 88 9A 99 59 40
        // decodes to kind=0x02, index=20 (MODEL_GAIN), value=3.4f. Literal, not a round trip - a
        // round-trip test passed against the old wrong code and is exactly what hid this bug.
        val fixture = byteArrayOf(
            0xB9.toByte(), 0x04, 0x02, 0x00, 0x14,
            0x88.toByte(), 0x9A.toByte(), 0x99.toByte(), 0x59, 0x40,
        )
    ...
    @Test
    fun `decode reads the index from a single byte at offset 4, ignoring offset 3 padding`() {
    ...
    // ---- encode and decode ARE inverses of each other, confirmed by hardware (#106) ----------------
    ...
    @Test
    fun `encode and decode agree for index 0`() {
